...
const EachBusReturn = (returnbustimings) => {

  const [returnactive, setReturnActive] = useState(0);
      ...
      <div className="bg-white">
        <div className="max-w-2xl mx-auto py-2 sm:py-4 lg:max-w-7xl">
          ...
          <div className="grid grid-cols-1 gap-y-4 md:grid-cols-2 sm:gap-x-2 sm:gap-y-10 lg:grid-cols-3 lg:gap-x-4">
            {returnbustimings.returnbustiming.map(availablereturnbus => (
              <button key={availablereturnbus.busid}
                className={classNames(returnactive ? "bg-yellow-100" : "", "focus:outline-none focus:ring focus:ring-yellow-100 rounded-lg"
                )}
                onClick={() => {
                  localstorage.setItem("usr_return_jr", JSON.stringify(availablereturnbus));
                  setReturnActive(availablereturnbus.busid);
                }}>
                <div className={classNames(availablereturnbus.busid === returnactive ? "bg-yellow-100" : "", "relative bg-white border border-gray-200 rounded-lg flex flex-col overflow-hidden flex-wrap")}
                >
                  <div className="p-3 space-y-2 flex flex-col bg-amber-700 text-slate-50 border border-amber-700 ">
                    <div className="grid grid-cols-3">
                      <p className="place-self-start sm:place-self-end font-bold">{availablereturnbus.journeystarttime.slice(0, 2) + ":" + availablereturnbus.journeystarttime.slice(2, 4)}</p>
                      <p className="place-self-center">&rarr;</p>
                      <p className="place-self-end sm:place-self-start font-bold">{availablereturnbus.journeyendtime.slice(0, 2) + ":" + availablereturnbus.journeyendtime.slice(2, 4)}</p>
                    </div>
                  </div>
                  <div>
                    <div className="grid grid-cols-1 text-lg m-4">
                      <ul className="flex flex-col space-y-2 mx-auto">
                        {
                          availablereturnbus.path.map(eachPath => (
                            <div key={eachPath.pathid}>
                              ...
                              {availablereturnbus.path.length - 1 >= eachPath.pathid ? (<hr className="border-violet-500" />) : (null)}
                            </div>
                          ))}
                      </ul>
                    </div>
